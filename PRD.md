...
- Target (aggressive/aspirational): aim for <0.05ms average server impact for core event handler overhead when profiled via resmon (pre-validation/logging/DB).
...
- Measurement guidance: profile with resmon/txAdmin over ≥5 minutes of representative duty scenarios (routine patrols, warrant work, 5–10 active officers and 20–30 total players) under typical player load; consider spikes acceptable if <5% of samples exceed the baseline target.
