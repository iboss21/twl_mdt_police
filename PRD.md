...
- Target (aggressive/aspirational): ideal-case benchmark of <0.05ms average server impact for core event handler overhead when profiled via resmon (pre-validation/logging/DB); adjust to what your hardware realistically supports.
...
- Measurement guidance: profile with resmon/txAdmin (sampling ~1s intervals) over ≥5 minutes of representative duty scenarios (routine patrols, warrant work, 5–10 active officers and 20–30 total players) under typical player load; consider spikes acceptable if <5% of samples exceed the baseline target.
