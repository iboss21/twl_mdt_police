    ...
    methods: {
        changePage(page) {
            ...
            if (page == "Home") {
                ...
                $.post('http://bucky_mdt/getWarrants');
        ...
        OffenderSearch() {
            if (this.offender_search) {
                ...
                $.post('http://bucky_mdt/performOffenderSearch', JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            for (var key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    ...
                    $.post('http://bucky_mdt/viewOffender', JSON.stringify({
        ...
        SaveOffenderChanges() {
            $.post('http://bucky_mdt/saveOffenderChanges', JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post('http://bucky_mdt/performReportSearch', JSON.stringify({
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && this.report_new.incident) {
                $.post('http://bucky_mdt/submitNewReport', JSON.stringify({
        ...
        DeleteSelectedReport() {
            $.post('http://bucky_mdt/deleteReport', JSON.stringify({
        ...
        SubmitNewWarrant() {
            ...
            $.post('http://bucky_mdt/submitNewWarrant', JSON.stringify({
        ...
        DeleteSelectedWarrant() {
            $.post('http://bucky_mdt/deleteWarrant', JSON.stringify({
        ...
        OpenReportById(id) {
            $.post('http://bucky_mdt/getReport', JSON.stringify({
                id: id
            }));
            this.modal = 'loading';
            return;
